...
def convert_cityscapes_instance_only(
    ...
    for data_set, ann_dir in zip(sets, ann_dirs):
        ...
        for root, _, files in os.walk(ann_dir):
            for filename in files:
                if filename.endswith(ends_in % data_set.split('_')[0]):
                    ...
                    image['file_name'] = os.path.join(root.split('/')[-1], image['file_name'])
                    image['seg_file_name'] = os.path.join(root.split('/')[-1], image['seg_file_name'])

                    images.append(image)

                    fullname = os.path.join('/'.join(root.split('/')[:-1]), image['seg_file_name'])
        ...
        print('save to:', os.path.join(out_dir, json_name % data_set))
