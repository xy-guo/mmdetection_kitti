...
def _rotation_box2d_jit_(corners, angle, rot_mat_T):
    """rotation boxes 2d, clockwise

    TODO: check the usage of this function, it seems that the function does not support batch mode
...
def box_collision_test(boxes, qboxes, clockwise=True):
    """box collision test, TODO: check the function of this code.

    :param boxes: array [N, 4, 2],  all the boxes
    :param qboxes: array [K, 4, 2], all the query boxes
    :param clockwise: boolean, default is True.
    :return: collision, boolean array [N, K]
    """
    N = boxes.shape[0]
    K = qboxes.shape[0]
    ret = np.zeros((N, K), dtype=np.bool_)
    # compute the line (two vertices) of each surface for boxes and qboxes
    slices = np.array([1, 2, 3, 0])
    lines_boxes = np.stack(
        (boxes, boxes[:, slices, :]), axis=2)  # [N, 4, 2(line), 2(xy)]
    lines_qboxes = np.stack((qboxes, qboxes[:, slices, :]), axis=2)
    # vec = np.zeros((2,), dtype=boxes.dtype)
    # boxes/qboxes_standup: the ranges of all dimensions. [N, 2+2] xmin, ymin, xmax, ymax
    boxes_standup = box_np_ops.corner_to_standup_nd_jit(boxes)
    qboxes_standup = box_np_ops.corner_to_standup_nd_jit(qboxes)
    for i in range(N):
        for j in range(K):
            # calculate standup first, iw and ih are the intersection width and height
            ...
            if iw > 0:
                ...
                if ih > 0:
                    # the area of the intersection is bigger than 0
                    # interate over each line (4 lines in total for squares)
                    for k in range(4):
                        for l in range(4):
                            A = lines_boxes[i, k, 0]
                            B = lines_boxes[i, k, 1]
...
def noise_per_box(boxes, valid_mask, loc_noises, rot_noises):
    ...
    box_corners = box_np_ops.box2d_to_corner_jit(boxes)  # [N, 4, 2]
    ...
    for i in range(num_boxes):
        if valid_mask[i]:
            for j in range(num_tests):
                ...
                current_corners -= boxes[i, :2]  # remove the center position
                _rotation_box2d_jit_(current_corners, rot_noises[i, j], rot_mat_T)  # do extra rotations
                current_corners += boxes[i, :2] + loc_noises[i, j, :2]  # add center position back and loc_noises
                coll_mat = box_collision_test(current_corners.reshape(1, 4, 2), box_corners)  # collision test
                coll_mat[0, i] = False
                # print(coll_mat)
                if not coll_mat.any():
                    success_mask[i] = j
...
def _rotation_matrix_3d_(rot_mat_T, angle, axis):
    """3d rotation matrix along a certain axis (x, y or z axis)"""
    rot_sin = np.sin(angle)
    rot_cos = np.cos(angle)
    rot_mat_T[:] = np.eye(3)
    if axis == 1:
...
def noise_per_object_v3_(gt_boxes, points=None, valid_mask=None, rotation_perturb=np.pi / 4, center_noise_std=1.0,
    ...
    if not enable_grot:
        ...
        selected_noise = noise_per_box_v2_(gt_boxes[:, [0, 1, 3, 4, 6]], valid_mask, loc_noises,
    ...
    if points is not None:
        ...
        point_masks = geometry.points_in_convex_polygon_3d_jit(points[:, :3], surfaces)
